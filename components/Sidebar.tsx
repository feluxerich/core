  ...
  return (
    <div className="h-screen bg-primary-800 flex flex-col items-center justify-between" style={{ width: '75px' }}>
      <div className="flex flex-col justify-start items-center w-full p-3">
        {Object.entries(sidebar).map((group, i) => {
          const groupName = group[0];
          const entries = group[1];

          return (
            <div className="w-full grid grid-cols-1 gap-3 mb-3" key={i}>
              {entries.map(({ icon: Icon, name, route }, i) => {
                return (
                  <Link href={route} passHref key={i}>
                    <div className="bg-primary-700 rounded-15 grid place-items-center w-full cursor-pointer" style={{ aspectRatio: '1 / 1' }}>
